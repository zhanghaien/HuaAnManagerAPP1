...
public class BaseImage {
    ...
    //直接加载网络图片/不裁剪
    public void displayImageNotCrop(Context context, String url, ImageView imageView,int resId) {
        Glide.with(context)
                .load(url)
                .placeholder(resId)
                .error(resId)
    ...
    //加载网络图片并设置大小
    public void displayImage(Context context, String url, ImageView imageView, int width, int height,int resId) {
        Glide.with(context)
                .load(url)
                .centerCrop()
                .thumbnail(0.1f)
    ...
    //加载网络图片显示为圆形图片
    public void displayCricleImage(Context context, String url, ImageView imageView,int resourceId) {
        Glide.with(context)
                .load(url)
                //.centerCrop()//网友反馈，设置此属性可能不起作用,在有些设备上可能会不能显示为圆形。
                .thumbnail(0.1f)
