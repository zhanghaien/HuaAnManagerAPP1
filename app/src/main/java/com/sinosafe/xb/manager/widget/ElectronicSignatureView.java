...
public class ElectronicSignatureView extends View {
    ...
    private static final String TAG = ElectronicSignatureView.class.getSimpleName();
     ...
     * @param blank      边缘空白区域
     */
    public void save(boolean clearBlank, int blank) throws IOException {
        Bitmap bitmap = cachebBitmap;
        //BitmapUtil.createScaledBitmapByHeight(srcBitmap, 300);//  压缩图片
        if (clearBlank) {
            bitmap = clearBlank(bitmap, blank);
        }
        FileUtil.saveImage(getContext(), SIGNATURE_NAME, bitmap);
    }
